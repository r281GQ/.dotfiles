...
searchAndReplaceInVisual = function(s, replace)
    local selection_pos = vim.fn.getpos('v')[2]

    local cursor_pos = vim.fn.getcurpos()[2]

    -- vim.cmd('startinsert')

    -- local d = vim.fn.search('sdfs', '', cursor_pos)
    ...
    if selection_pos > cursor_pos then
        r = cursor_pos
        ee = selection_pos
    else
        r = selection_pos
    ...
    vim.fn.cursor({r - 1, 0, 0})

    -- local d = vim.fn.search('selection_pos', '', ee)
    local d = vim.fn.search('cursor_pos', '', ee)
    ...
    local input = Input({
        ...
        border = {
            style = "single",
            text = {top = "How old are you?", top_align = "center"}
        },
